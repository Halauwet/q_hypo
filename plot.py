...
jm=15
dur=0
tzsign=''; tzone=0; tzname='UTC'; periode=''
ot_start=0; ot_stop=0; start_time=0; stop_time=0
jumlah_event=0; jumlah_dirasakan=0
...
while i < len(baris):
	...
	if len(baris[i])>0 and baris[i][0]=='Plot_Animasi' and baris[i+1][0]=='Y' or len(baris[i])>0 and baris[i][0]=='Plot_Animasi' and baris[i+1][0]=='y':
		plot_animasi='YES'
	if len(baris[i])>0 and baris[i][0]=='Plot_Dirasakan' and baris[i+1][0]=='Y' or len(baris[i])>0 and baris[i][0]=='Plot_Dirasakan' and baris[i+1][0]=='y':
		plot_dirasakan='YES'
...
ll_inset, rl_inset, bl_inset, ul_inset, logo_y1, logo_y2, logo_y3, kompas_x, kompas_y, skala_x, skala_y, skala_z, timepos_x, timepos_y, creditpos_x, creditpos_y, tinggi_map = set_pos(ll, rl, bl, ul, jm)
if plot_crossect=='YES':
	print('')
	...
	file.write(':PROJECT'+'\n')
	file.write('gawk "{print $6, $7, $8, $9, $10}" %tmpdata% | project  -L-5/5 -C%epic% -A%azm1% -Fxyzpqrs -Q > %proyeksi1%'+'\n')
	file.write('gawk "{print $6, $7, $8, $9, $10}" %tmpdata% | project  -L-5/5 -C%epic% -A%azm2% -Fxyzpqrs -Q > %proyeksi2%'+'\n')
	if plot_topo=='YES':
	...
	if plot_animasi=='YES':
		...
		while i < transp_max:
			if i==0:
				...
				strstep=strstep+'-%tpf%'
				file.write('gawk "{if ($10>%starttime%-%tpf%'+strstep+' && $10<=%starttime%'+strstep+') print $6,$7,$8,'+M_size+'}" %tmpdata% | psxy -R -JM -Sc -C%D% -W0.2 -O -t'+str(i)+' -K >> %ps%'+'\n')
			i+=transp_step
		file.write('gawk "{if ($10<%starttime%) print $6,$7,$8,'+M_size+'}" %tmpdata% | psxy -R -JM -Sc -C%D% -W0.25 -O -t'+str(transp_max)+' -K >> %ps%'+'\n')	
	else:
	...
	file.write('psscale -Dg'+str(jx_legend/2)+'/'+str(jx_legend/3)+'+w'+str(jx_legend*0.7)+'/'+str(jx_legend/16)+'+e+macl+h+jCB -J -R -Bx100+l"Kedalaman (km)" -G0/500 -I1 --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=6 --FONT_LABEL=6 -C%D% -N -O -K >> %ps%'+'\n')
	...
	if plot_topo=='YES':
		file.write(':CROSS_AB'+'\n')
		file.write('psxy %trackAB% -R%R4% -JX%boxwidht%/%boxelev% -BWe -Bya%maxelev%f1+l"Elev (km)" --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 -W3p -X-'+str(8.5+(8-jx_legend)/2)+' -Y-2 -O -K >> %ps%'+'\n')
		...
		file.write('psbasemap -JX%boxwidht%/%boxheight% -R%R2% -Bxa20+l"Distance (km)" -Bya20f20+l"Depth (km)" -BWSne+t"Cross Section Fault Plane 1" --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 --FONT_TITLE=10 -O -K -Y-%boxoverlap% >> %ps%'+'\n')
		if plot_animasi=='YES':
			i=0
			strstep=''
			while i < transp_max:
				if i==0:
					...
					strstep=strstep+'-%tpf%'
					file.write('gawk "{if ($5>%starttime%-%tpf%'+strstep+' && $5<=%starttime%'+strstep+') print $7, $3*(-1.0), $3, '+M_size+'}" %proyeksi1% | psxy -R -J -Sc -C%D% -W0.2 -O -t'+str(i)+' -K >> %ps%'+'\n')
				i+=transp_step
			file.write('gawk "{if ($5<%starttime%) print $7, $3*(-1.0), $3, '+M_size+'}" %proyeksi1% | psxy -R -J -Sc -C%D% -W0.2 -O -t'+str(transp_max)+' -K >> %ps%'+'\n')
		else:
		...
		file.write(':CROSS_CD'+'\n')
		file.write('psxy %trackCD% -R%R4% -JX%boxwidht%/%boxelev% -Bew -Bya%maxelev%f1 --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 -W3p -X8.5 -Y%boxoverlap% -O -K >> %ps%'+'\n')
		...
		file.write('echo %length% 0 >> %sealevel%'+'\n')
		file.write('psxy %sealevel% -J -R -W1,89/89/171 -O -K >> %ps%'+'\n')
		file.write('psbasemap -JX%boxwidht%/%boxheight% -R%R2% -Bxa20+l"Distance (km)" -BeSnw+t"Cross Section Fault Plane 2" --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 --FONT_TITLE=10 -O -K -Y-%boxoverlap%>> %ps%'+'\n')
		if plot_animasi=='YES':
			i=0
			strstep=''
			while i < transp_max:
				if i==0:
					...
					strstep=strstep+'-%tpf%'
					file.write('gawk "{if ($5>%starttime%-%tpf%'+strstep+' && $5<=%starttime%'+strstep+') print $7, $3*(-1.0), $3, '+M_size+'}" %proyeksi2% | psxy -R -J -Sc -C%D% -W0.2 -O -t'+str(i)+' -K >> %ps%'+'\n')
				i+=transp_step
			file.write('gawk "{if ($5<%starttime%) print $7, $3*(-1.0), $3, '+M_size+'}" %proyeksi2% | psxy -R -J -Sc -C%D% -W0.2 -O -t'+str(transp_max)+' -K >> %ps%'+'\n')
		else:
		...
		file.write(':CROSS_AB'+'\n')
		file.write('psbasemap -JX%boxwidht%/%boxheight% -R%R2% -BWSne+t"Cross Section Fault Plane 1" -Bxa20+l"Distance (km)" -Bya20f20+l"Depth (km)" --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 --FONT_TITLE=10 -O -K -X-'+str(8.5+(8-jx_legend)/2)+' -Y-5 >> %ps%'+'\n')
		...
		file.write(':CROSS_CD'+'\n')
		file.write('psbasemap -JX%boxwidht%/%boxheight% -R%R2% -BeSnw+t"Cross Section Fault Plane 2" -Bxa20+l"Distance (km)" --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 --FONT_TITLE=10 -O -K -X8.5 >> %ps%'+'\n')
	...
	if lblperiode=='Periode kurang':
		...
		file.write('psbasemap -JX5/%boxheight% -R/0/1/0/1 -Bg1x --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 --FONT_TITLE=10 -O -K -Y'+str(graf_yoffset)+' >> %ps%'+'\n')
			####
		if plot_animasi=='YES':
			file.write('gawk "{if ($10<=%starttime%) print '+tab_data+'$4}" %tmpdata% | pshistogram -JX5/%boxheight% -W1+b -Gblue -L -BESwn -Bx+l"'+lblperiode+'" -By+l"Jumlah Gempabumi" -F --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 --FONT_TITLE=10 -O -K  >> %ps%'+'\n')
		else:
			file.write('gawk "{print '+tab_data+'}" %tmpdata% | pshistogram -JX5/%boxheight% -W1+b -Gblue -L -BESwn -Bx+l"'+lblperiode+'" -By+l"Jumlah Gempabumi" -F --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 --FONT_TITLE=10 -O -K >> %ps%'+'\n')
	...
	if plot_animasi=='YES':
		...
		file.write('ffmpeg -framerate %fps% -i frame/frame-%%d.png -c:v libx264 -profile:v high -crf 18 -pix_fmt yuv420p -vf "scale=1080:ceil(ih/(iw/1080*2))*2" %output%'+'\n')
	...
	if plot_animasi=='YES':
		...
		while i < transp_max:
			if i==0:
				...
				if plot_dirasakan=='YES':
					file.write('gawk "{if ($10>%starttime%-%tpf% && $10<=%starttime%) print $6,$7,$8,'+M_size+'}" %tmpdirasakan% | psxy -R -JM -Sa -C%D% -W0.5,white -O -K >> %ps%'+'\n')
			else:
				strstep=strstep+'-%tpf%'
				file.write('gawk "{if ($10>%starttime%-%tpf%'+strstep+' && $10<=%starttime%'+strstep+') print $6,$7,$8,'+M_size+'}" %tmpdata% | psxy -R -JM -Sc -C%D% -W0.25 -O -t'+str(i)+' -K >> %ps%'+'\n')
				if plot_dirasakan=='YES':
					file.write('gawk "{if ($10>%starttime%-%tpf%'+strstep+' && $10<=%starttime%'+strstep+') print $6,$7,$8,'+M_size+'}" %tmpdirasakan% | psxy -R -JM -Sa -C%D% -W0.5,white -O -t'+str(i)+' -K >> %ps%'+'\n')
			i+=transp_step
		file.write('gawk "{if ($10<%starttime%) print $6,$7,$8,'+M_size+'}" %tmpdata% | psxy -R -JM -Sc -C%D% -W0.25 -O -t'+str(transp_max)+' -K >> %ps%'+'\n')
		if plot_dirasakan=='YES':
			file.write('gawk "{if ($10<%starttime%) print $6,$7,$8,'+M_size+'}" %tmpdirasakan% | psxy -R -JM -Sa -C%D% -W0.5,white -O -t'+str(transp_max)+' -K >> %ps%'+'\n')
		...
		if plot_dirasakan=='YES':
			file.write('gawk "{print $6,$7,$8,'+M_size+'}" %tmpdirasakan% | psxy -R -JM -Sa -C%D% -W0.5,white -O -K >> %ps%'+'\n')
	...
	file.write('psscale -Dg'+str(jx_legend/2)+'/'+str(jx_legend/3)+'+w'+str(jx_legend*0.8)+'/'+str(jx_legend/16)+'+e+macl+h+jCB -J -R -Bx100+l"Kedalaman (km)" -G0/500 -I1 --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=6 --FONT_LABEL=6 -C%D% -O -K >> %ps%'+'\n')
	file.write('psxy %L% -R -J -Sc -W0.35 -O -K >> %ps%'+'\n')
	file.write('gawk "{print $1,$4,$5}" %L% | pstext -J -R -F+f6p,Helvetica,black+jCB -O -K >> %ps%'+'\n')
	...
	if lblperiode=='Periode kurang':
		...
		if plot_animasi=='YES':
			file.write('gawk "{if ($10<=%starttime%) print $8}" %tmpdata% | pshistogram -JX5/'+str(tinggi_graph)+' -W20+b -Gblue -L -Z4 -BESwn -Bx+l"Kedalaman (km)" -By+l"Log(Jumlah Gempabumi)" --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 --FONT_TITLE=10 -O -K  >> %ps%'+'\n')
		else:
			file.write('gawk "{print $8}" %tmpdata% | pshistogram -JX5/'+str(tinggi_graph)+' -W20+b -Gblue -L -Z4 -BESwn -Bx+l"Kedalaman (km)" -By+l"Log(Jumlah Gempabumi)" --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 --FONT_TITLE=10 -O -K >> %ps%'+'\n')
		file.write('psbasemap -JX5/'+str(tinggi_graph)+' -R/0/1/0/1 -Bg1x --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 --FONT_TITLE=10 -O -K -Y'+str(graf_yoffset)+' >> %ps%'+'\n')
		if plot_animasi=='YES':
			####
			file.write('gawk "{if ($10<=%starttime%) print '+tab_data+'}" %tmpdata% | pshistogram -JX5/'+str(tinggi_graph)+' -W1+b -Gblue -L -BESwn -Bx+l"'+lblperiode+'" -By+l"Jumlah Gempabumi" -F --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 --FONT_TITLE=10 -O -K  >> %ps%'+'\n')
		else:
			####
			file.write('gawk "{print '+tab_data+'}" %tmpdata% | pshistogram -JX5/'+str(tinggi_graph)+' -W1+b -Gblue -L -BESwn -Bx+l"'+lblperiode+'" -By+l"Jumlah Gempabumi" -F --MAP_ANNOT_MIN_SPACING=0.1p --FONT_ANNOT_PRIMARY=8 --FONT_LABEL=9 --FONT_TITLE=10 -O -K >> %ps%'+'\n')
		file.write('echo 0.9 -0.24 "by: @@eqhalauwet" | pstext -R -Bg1x -J -F+f9,ZapfChancery-MediumItalic+jLT -Y-'+str(graf_yoffset*2)+' -N -O >> %ps%'+'\n')
	if plot_animasi=='YES':
		file.write('psconvert %ps% -Tg -E'+str(dpi)+' -P -A0.2 -Fframe/frame-%framenumber%'+'\n')
		file.write('goto loopplot'+'\n')
		...
		file.write('echo %title% >> ../tmp/judul'+'\n')
		file.write('pstext ../tmp/judul -R -J -F+f12p,NewCenturySchlbk-BoldItalic,black+jCB -Gwhite -M -t20 -O -K >> %ps%'+'\n')
		...
		file.write('echo '+str(skala_x)+' '+str(logo_y2)+' %fromdate% hingga %todate% | pstext -R -J -F+f10,NewCenturySchlbk-BoldItalic,black+jCB -Gwhite -N -t20 -O -K >> %ps%'+'\n')
		# if plot_dirasakan=='NO':
			# file.write('echo '+str(skala_x)+' '+str(logo_y3)+' Jumlah gempabumi '+jumlah_event+' kejadian | pstext -R -J -F+f9,NewCenturySchlbk-BoldItalic,black+jCM -Gwhite -N -t20 -O -K >> %ps%'+'\n')
		# else:
		if jumlah_dirasakan==0:
			file.write('echo '+str(skala_x)+' '+str(logo_y3)+' Jumlah gempabumi '+jumlah_event+' kejadian, tidak ada laporan dirasakan oleh masyarakat | pstext -R -J -F+f9,NewCenturySchlbk-BoldItalic,black+jCM -Gwhite -N -t20 -O -K >> %ps%'+'\n')
		else:
			file.write('echo '+str(skala_x)+' '+str(logo_y3)+' Jumlah gempabumi '+jumlah_event+' kejadian, '+jumlah_dirasakan+' dilaporkan dirasakan oleh masyarakat | pstext -R -J -F+f9,NewCenturySchlbk-BoldItalic,black+jCM -Gwhite -N -t20 -O -K >> %ps%'+'\n')
		file.write('pscoast -R%R_inset% -JM4 -Dh -W0.2p,black -Gwhite -S150/255/255 -B5::wsNE --MAP_FRAME_TYPE=plain --FONT_ANNOT_PRIMARY=6 -O -K >> %ps%'+'\n')
		# Plot Legenda
		R, J = set_legend(jx_legend, num_M, min_M, legenda, M_scale, float(M_linear_size))
		file.write('psbasemap -R'+R+' -JX'+J+' -Bx --FONT_ANNOT_PRIMARY=6 -O -K -X'+str(jm-jx_legend)+' >> %ps%'+'\n')
